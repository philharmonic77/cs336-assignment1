...
from cs336_basics.text import Tokenizer
...
from cs336_basics.training import gradient_clipping, learning_rate_schedule, save_checkpoint, load_checkpoint
...
def run_linear(
    ...
    linear_layer = Linear(d_in, d_out, weights.device, weights.dtype)
    result = linear_layer.load_state_dict({"weight": weights})
    print(result)
...
def run_transformer_block(
    ...
    result = block.load_state_dict(weights)
    print(result)
...
def run_transformer_lm(
    ...
    """
    model = TransformerLM(vocab_size, context_length, num_layers, d_model, num_heads, d_ff, rope_theta, device=in_indices.device, dtype=torch.float32)
    result = model.load_state_dict(weights)
    print(result)
...
def run_rmsnorm(
    ...
    result = rmsnorm_layer.load_state_dict({"weight": weights})
    print(result)
